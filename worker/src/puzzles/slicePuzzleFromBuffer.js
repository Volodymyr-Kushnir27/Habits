...
function getPatternConfig(pattern) {
  switch (pattern) {
    case 'rounded':
      return { knobScale: 0.25, neckScale: 0.14, shoulderScale: 0.23, jitter: 0.015 };
    case 'deep':
      return { knobScale: 0.39, neckScale: 0.12, shoulderScale: 0.2, jitter: 0.02 };
    case 'wavy':
      return { knobScale: 0.31, neckScale: 0.13, shoulderScale: 0.21, jitter: 0.05 };
    case 'irregular':
      return { knobScale: 0.34, neckScale: 0.12, shoulderScale: 0.18, jitter: 0.09 };
    case 'classic':
    default:
      return { knobScale: 0.31, neckScale: 0.13, shoulderScale: 0.22, jitter: 0.02 };
  }
}
...
function buildPiecePath({ row, col, cellWidth, cellHeight, offsetX, offsetY, edgeMap, cfg, rng }) {
  const left = offsetX;
  const top = offsetY;
  ...
  const topEdge =
    row === 0
      ? { sign: 0, variance: 1 }
      : {
          sign: -edgeMap.horizontal[row - 1][col].sign,
          variance: edgeMap.horizontal[row - 1][col].variance,
        };
  ...
  const leftEdge =
    col === 0
      ? { sign: 0, variance: 1 }
      : {
          sign: -edgeMap.vertical[row][col - 1].sign,
          variance: edgeMap.vertical[row][col - 1].variance,
        };

  const path = [`M ${left.toFixed(2)} ${top.toFixed(2)}`];

  drawTopEdge(path, left, top, cellWidth, topEdge.sign, getEdgeMetrics(cellWidth, topEdge.variance, cfg, rng));
  drawRightEdge(path, right, top, cellHeight, rightEdge.sign, getEdgeMetrics(cellHeight, rightEdge.variance, cfg, rng));
  drawBottomEdge(path, right, bottom, cellWidth, bottomEdge.sign, getEdgeMetrics(cellWidth, bottomEdge.variance, cfg, rng));
  drawLeftEdge(path, left, bottom, cellHeight, leftEdge.sign, getEdgeMetrics(cellHeight, leftEdge.variance, cfg, rng));
...
export async function slicePuzzleFromBuffer({
  imageBuffer,
  piecesFolder,
  patternSeedKey,
  uploadPieceFn,
  ...
  if (!imageBuffer || !Buffer.isBuffer(imageBuffer)) {
    throw new Error('slicePuzzleFromBuffer: imageBuffer is invalid');
  }
  ...
  const pattern = pickPattern(patternSeedKey);
  const cfg = getPatternConfig(pattern);

  const edgeMap = buildEdgeMap(GRID_COLS, GRID_ROWS, pattern, patternSeedKey);
  const rng = mulberry32(hashString(`rng_${patternSeedKey}`));
  ...
  for (let row = 0; row < GRID_ROWS; row++) {
    for (let col = 0; col < GRID_COLS; col++) {
      ...
      const padX = cellWidth * 0.45;
      const padY = cellHeight * 0.45;

      const extractLeft = Math.max(0, Math.floor(slotX - padX));
      const extractTop = Math.max(0, Math.floor(slotY - padY));
      const extractRight = Math.min(width, Math.ceil(slotX + cellWidth + padX));
      const extractBottom = Math.min(height, Math.ceil(slotY + cellHeight + padY));

      const extractWidth = extractRight - extractLeft;
      const extractHeight = extractBottom - extractTop;

      const offsetX = slotX - extractLeft;
      const offsetY = slotY - extractTop;
      ...
      const piecePng = await sharp(normalizedBuffer)
        .extract({
          left: extractLeft,
          top: extractTop,
          width: extractWidth,
      ...
      const piecePath = `${piecesFolder}/${pieceIndex}.png`;
      await uploadPieceFn(piecePath, piecePng);
  ...
  return {
    ...
    cutPattern: pattern,
    pieceAspectRatio: Number((cellWidth / cellHeight).toFixed(4)),
    manifest,
  };
}