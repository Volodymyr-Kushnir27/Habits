...
// розмір "зубця"
const TAB_SIZE_RATIO = 0.22; // 22% від розміру клітинки
...
function randomSign() {
  return Math.random() > 0.5 ? 1 : -1; // 1 = назовні, -1 = всередину
}
...
async function addFreeCatalogPuzzle({
  ...
  if (!meta.width || !meta.height) {
    throw new Error('Не вдалося визначити розмір картинки');
  }
...
const sortOrder = Number(process.argv[5] || 1);

if (!originalPath || !title) {
  console.log(`
Використання:
node scripts/addFreeCatalogPuzzle.js "1.png" "Козаки • Вечір" "ukrainian cozy evening" 1
`);
  process.exit(1);
}

addFreeCatalogPuzzle({
  originalPath,
  title,
  prompt,
  sortOrder,
